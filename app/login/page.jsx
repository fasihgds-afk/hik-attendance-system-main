...
function LoginInner() {
  ...
  async function handleHrSubmit(e) {
    ...
    const email = hrEmail.trim();
    if (!email || email.length > 254 || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
      setErrorMsg("Enter a valid email address.");
      return;
    }
    if (!hrPassword || hrPassword.length > 128) {
  ...
  async function handleEmployeeSubmit(e) {
    ...
    if (!code || code.length > 20 || !/^[A-Za-z0-9]+$/.test(code)) {
      setErrorMsg("Employee code must be letters and numbers only (max 20).");
      return;
    }
                ...
                <form className="login-form" onSubmit={handleHrSubmit} style={formShellStyle}>
                  <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                    ...
                    <input
                      type="email"
                      value={hrEmail}
                      onChange={(e) => setHrEmail(e.target.value.slice(0, 254))}
                      required
                      maxLength={254}
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                    ...
                    <div className="login-password-wrap" style={{ position: "relative", width: "100%" }}>
                      <input
                        ...
                        value={hrPassword}
                        onChange={(e) => setHrPassword(e.target.value.slice(0, 128))}
                        required
                        maxLength={128}
                ...
                >
                  <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                    ...
                    <input
                      value={empCode}
                      onChange={(e) =>
                        setEmpCode(e.target.value.replace(/[^A-Za-z0-9]/g, "").slice(0, 20))
                      }
                      required
                      maxLength={20}
                      inputMode="text"
