  ...
  providers: [
    CredentialsProvider({
      ...
      async authorize(credentials) {
        ...
        if (mode === "EMPLOYEE") {
          const parsed = parseEmployeeLogin({ empCode: credentials.empCode });
          if (!parsed.ok) return null;
          const { empCode: code } = parsed.data;
...
async function handler(req, context) {
  // Rate-limit credential POSTs so brute-force cannot hammer MongoDB via NextAuth.
  if (req.method === "POST") {
