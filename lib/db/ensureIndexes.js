...
export async function ensureAllIndexes() {
  try {
    ...
    const [
      ...
      { default: LeavePolicy },
      { default: AttendanceEvent },
      { default: Complaint },
      { default: LeaveRecord },
      { default: EmployeeShiftHistory },
      { default: PaidLeave },
      { default: MonthlyAttendance },
    ] = await Promise.all([
      ...
      import('../../models/EmployeeShiftHistory'),
      import('../../models/PaidLeave'),
      import('../../models/MonthlyAttendance'),
    ]);
    ...
    try {
      ...
      console.warn('Employee status backfill warning:', e.message);
    }
    ...
    const results = await Promise.allSettled([
      ...
      AttendanceEvent.createIndexes().catch(err => {
        if (err.message?.includes('already exists')) {
          return Promise.resolve();
        }
        throw err;
      ...
      LeaveRecord.createIndexes().catch(err => {
        if (err.message?.includes('already exists')) {
          return Promise.resolve();
        }
        throw err;
      }),
      EmployeeShiftHistory.createIndexes().catch(err => {
        if (err.message?.includes('already exists')) {
          return Promise.resolve();
        }
        throw err;
      }),
      PaidLeave.createIndexes().catch(err => {
        if (err.message?.includes('already exists')) {
          return Promise.resolve();
        }
        throw err;
      }),
      MonthlyAttendance.createIndexes().catch(err => {
        if (err.message?.includes('already exists')) {
          return Promise.resolve();
        }
        throw err;
