...
export function optimizeEmployeeSearch(searchTerm) {
  ...
  // - use prefix regex for indexed fields (empCode/email) to stay index-friendly
  // - keep a contains regex on name for user-friendly matching
  return {
    $or: [
      { empCode: { $regex: `^${escaped}`, $options: 'i' } }, // prefix match on indexed field
      { email: { $regex: `^${escaped}`, $options: 'i' } }, // prefix match on indexed field
