...
/** Side-specific border only — never set `border` or `borderColor` with `borderTopColor` (React warning). */
function solidBorder({ color, top, bottom, left, right, width = '1px', style = 'solid' }) {
  const base = color ?? 'transparent';
  return {
    borderWidth: width,
    borderStyle: style,
    borderTopColor: top ?? base,
    borderRightColor: right ?? base,
    borderBottomColor: bottom ?? base,
    borderLeftColor: left ?? base,
  };
}

/** Loading spinner ring without border shorthand conflicts */
export function spinnerRingStyle(trackColor, activeColor, width = '2px') {
  return {
    borderWidth: width,
    borderStyle: 'solid',
...
export function getCardStyles(themeOrColors = 'dark', options = {}) {
  ...
  const { padding = 24, borderRadius = 16, muted = false } = options;
  const borderColor = colors.glass.border;
  const topBorder = muted ? borderColor : colors.glass.gloss?.rimTop || borderColor;
  const bottomBorder = muted
    ? colors.glass.borderBottom || borderColor
    : colors.glass.borderBottom || borderColor;
  ...
  return {
    ...
    boxShadow: muted
      ...
      : colors.glass.shadow,
    ...(muted ? {} : glassBackdrop(colors)),
    color: colors.glass.text,
  };
}

/** Accent strip without mixing borderLeft shorthand against solidBorder longhands */
export function withLeftAccent(style, accentColor, widthPx = 4) {
  return {
    ...style,
