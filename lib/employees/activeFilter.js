...
/** Indexed-friendly: missing/null status counts as active ($nin matches missing fields). */
export const ACTIVE_EMPLOYEE_FILTER = {
  status: { $nin: [EMPLOYEE_STATUS.INACTIVE, EMPLOYEE_STATUS.TERMINATED] },
};
...
export function mergeActiveFilter(filter = {}) {
  ...
  // $text must appear at the top level of the query document
  if (filter.$text) {
    const { $text, ...rest } = filter;
    if (Object.keys(rest).length === 0) {
      return { $text, ...ACTIVE_EMPLOYEE_FILTER };
    }
    return { $text, $and: [ACTIVE_EMPLOYEE_FILTER, rest] };
  }
...
export function mergeArchivedFilter(filter = {}) {
  ...
  if (filter.$text) {
    const { $text, ...rest } = filter;
    if (Object.keys(rest).length === 0) {
      return { $text, ...ARCHIVED_EMPLOYEE_FILTER };
    }
    return { $text, $and: [ARCHIVED_EMPLOYEE_FILTER, rest] };
  }
...
export async function fetchEmployeesForMonthlySheet(
  ...
  if (monthRelation >= 0) {
    return Employee.find(mergeActiveFilter({}))
      .select(select)
      .lean()
      .maxTimeMS(maxTimeMS);
  }

  const [activeEmployees, codesInMonth] = await Promise.all([
    Employee.find(mergeActiveFilter({}))
      .select(select)
      .lean()
      .maxTimeMS(maxTimeMS),
    ShiftAttendance.distinct('empCode', {
