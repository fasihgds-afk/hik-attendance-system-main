 ...
 * Counted minutes are clipped to each employee's own shift window
 * (forgotten / left-open sessions do not count pre/post shift hours).
 */

import {
  resolveShiftDateForBreak,
  allocateBreakToShiftDays,
} from '../shift/resolveShiftWindow.js';
 ...
 * Wall-clock only — prefer allocateBreakCountedSegments when a shift is known.
 */
 ...
 * or the break never overlaps the shift window.
 */
export function resolveBreakShiftDate({
  breakAt,
  breakEndAt = null,
  shift,
  ...
  if (shift) {
    const resolved = resolveShiftDateForBreak({
      breakAt,
      breakEndAt,
      shift,
      timezoneOffset,
...
/**
 * Minutes that count toward monitoring for each overlapping shift day.
 * Clips to that employee's shift window (e.g. N2 9pm–6am).
 * Empty when the break never overlaps any shift window.
 */
export function allocateBreakCountedSegments({
  startAt,
  endAt = null,
  shift,
  timezoneOffset = TZ_OFFSET,
  ...
  if (!shift?.startTime || !shift?.endTime) {
    const shiftDate = toLocalDateStr(startAt, timezoneOffset);
    const end = endAt && !Number.isNaN(endAt.getTime()) ? endAt : now;
    const minutes = Math.max(0, (end.getTime() - startAt.getTime()) / 60_000);
    return shiftDate ? [{ shiftDate, durationMin: minutes }] : [];
  }

  return allocateBreakToShiftDays({
    startAt,
    endAt,
    shift,
    timezoneOffset,
