...
export const employeeSchema = z.object({
  ...
  joinDate: z.preprocess(nullToUndef,
    z.string()
  ...
  bankDetails: z.preprocess(
    nullToUndef,
    z.object({
      bankName: z.preprocess(nullToUndef, z.string().max(120).optional().or(z.literal(''))),
      accountTitle: z.preprocess(nullToUndef, z.string().max(120).optional().or(z.literal(''))),
      accountNumber: z.preprocess(
        nullToUndef,
        z.string().max(34).regex(/^[A-Za-z0-9\-\/ ]*$/, 'Invalid account number format').optional().or(z.literal(''))
      ),
      iban: z.preprocess(
        nullToUndef,
        z
          .string()
          .max(34)
          .regex(/^[A-Za-z]{2}[A-Za-z0-9 ]{13,32}$/, 'Invalid IBAN format')
          .optional()
          .or(z.literal(''))
      ),
    }).optional(),
  ...
  allowWebClockIn: z.preprocess((val) => {
