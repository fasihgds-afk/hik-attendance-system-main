...
export async function getShiftsForEmployeesInDateRange(empCodes, startDate, endDate, options = {}) {
  ...
  for (const empCode of empCodesList) {
    ...
    const empHistory = history.filter((h) => toKey(h.empCode) === empCode);

    for (const date of dates) {
      let shiftCode = '';
      const applicable = empHistory.filter(
        (h) => h.effectiveDate <= date && (h.endDate == null || h.endDate >= date)
      );
      if (applicable.length > 0) {
        applicable.sort((a, b) => b.effectiveDate.localeCompare(a.effectiveDate));
        shiftCode = applicable[0].shiftCode || '';
      }
