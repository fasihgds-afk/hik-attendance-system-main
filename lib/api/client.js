...
export async function apiClient(url, options = {}) {
  const {
    ...
    headers = {},
    abortDuplicate = true,
    /** Stable key so a newer list/search request aborts the previous one (even when URL params differ). */
    requestKey: customRequestKey = null,
  ...
  // Check for duplicate / superseded requests
  const requestKey = customRequestKey || getRequestKey(method, fullUrl, body);
  ...
  try {
    ...
    // Only clear if this call still owns the key (a newer request may have replaced us)
    ...
    // If a newer request superseded this one, ignore the stale response
    if (activeRequests.get(requestKey) && activeRequests.get(requestKey) !== controller) {
      return {
        success: false,
    ...
    // Clear timeout
    clearTimeout(timeoutId);
    // Only remove if we still own this key — do not delete a newer request's controller
    if (activeRequests.get(requestKey) === controller) {
      activeRequests.delete(requestKey);
    }

    // Handle abort errors (timeout or superseded by a newer request)
