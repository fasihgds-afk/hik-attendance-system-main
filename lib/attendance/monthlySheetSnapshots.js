...
export async function upsertMonthlySheetSnapshot(
  month,
  { daysInMonth, employees, companyTodayYmd = '', force = false }
) {
  const monthKey = toMonthKey(month);
  if (!monthKey || !Array.isArray(employees)) return { ok: false };

  // Safety: never replace a full HR sheet with a tiny/empty payload
  // (e.g. employee-scoped compute if a caller forgets to skip upsert).
  if (!force) {
    try {
      const existing = await MonthlySheetSnapshot.findOne({ month: monthKey })
        .select('employeeCount')
        .lean()
        .maxTimeMS(2000);
      const prevCount = Number(existing?.employeeCount || 0);
      const nextCount = employees.length;
      if (prevCount >= 5 && nextCount < Math.max(2, Math.floor(prevCount * 0.5))) {
        console.warn(
          `[monthlySheetSnapshots] refused shrink ${monthKey}: ${prevCount} → ${nextCount}`
        );
        return { ok: false, reason: 'refuse_shrink', prevCount, nextCount };
      }
    } catch {
      // If the guard check fails, continue with upsert rather than blocking HR.
    }
  }
