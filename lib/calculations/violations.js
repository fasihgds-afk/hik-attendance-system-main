 ...
 * Grace period default comes from Shift model (single source of truth).
 * 
 * CONFIGURATION:
 * - Modify violation rules and fine rates here
...
} from '../shift/gracePeriods.js';
import { parseOffsetToMinutes } from '../time/companyTodayCore.js';

// Company timezone offset. Parsed properly so ANY offset works (not just +05:00),
// and matches the parser used by the monthly attendance engine for consistency.
const COMPANY_OFFSET_MS =
  parseOffsetToMinutes(process.env.TIMEZONE_OFFSET || '+05:00') * 60 * 1000;
 ...
 * 
 * @param {Object|String} shift - Shift object (with startTime, endTime, grace fields) or shift code string
...
export function computeLateEarly(
  ...
  // NOTE: this mirrors the monthly attendance engine EXACTLY so web-clock and the
  // monthly payroll sheet always agree on late/early violations.
  if (crossesMidnight) {
    const startClock = startMin % (24 * 60); // e.g. 21:00 = 1260
    const earlyMorningThreshold = 6 * 60; // 06:00 = 360

    // Normalize shift end to next day only when end clock-time is before start clock-time.
    endMin = rawEndMin < startClock ? rawEndMin + 24 * 60 : rawEndMin;

    // Early-morning check-in (00:00–05:59) belongs to the shift that started the previous day.
    if (inMin < earlyMorningThreshold) {
      inMin += 24 * 60;
    }

    // Normalize check-out to next day when its clock-time is before the shift start clock-time.
...
/** Re-export for consumers that import from violations */
export { DEFAULT_GRACE_PERIOD } from '../shift/gracePeriods.js';
