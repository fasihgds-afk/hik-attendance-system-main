...
// Prefer public DNS — Windows/ISP resolvers often refuse MongoDB SRV lookups
try {
...
// Load .env / .env.local manually if not set
if (!process.env.MONGO_URI) {
  for (const file of ['.env.local', '.env']) {
    ...
    for (const line of content.split(/\r?\n/)) {
      ...
      if (m) {
        process.env.MONGO_URI = m[1].replace(/^["']|["']$/g, '');
        break;
      }
    }
...
async function test() {
  ...
  if (!uri) {
    console.error('❌ MONGO_URI not found in .env or .env.local');
